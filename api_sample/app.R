...
# Source de los módulos de api_sample
# (Asegúrate de que la ruta coincide con tu archivo de módulo de tamaño muestral)
...
ui <- page_navbar(
  ...
  nav_panel(
    title = span(id = "sub_sample_placeholder", "Tamaño Muestral"),
    # Asegúrate de usar el ID correcto de tu UI function
    tamano_muestral_UI("sample_module") 
  )
)

server <- function(input, output, session) {
  message("DEBUG: Server session started (api_sample)")
  
  # Captura Reactiva estandarizada
  lang <- reactive({
    l <- if (is.null(input$global_lang)) "es" else input$global_lang
    l
  })

  # Diccionario base para Home y Navegación
  nav_tr <- list(
    es = list(
      ...
      sub_sample_placeholder = "Tamaño Muestral"
    ),
    en = list(
      home_tab_label = "Home",
      ...
      sub_sample_placeholder = "Sample Size"
    ),
    pt = list(
      ...
      print_btn_print = "Imprimir",
      sub_sample_placeholder = "Tamanho da Amostra"
    ),
    zh = list(
      home_tab_label = "首页",
      ...
      print_btn_close = "关闭",
      print_btn_print = "打印",
      sub_sample_placeholder = "样本量"
    )
  )

  # Actualizar dinámicamente títulos para scripts y frontend
  observeEvent(lang(), {
    t <- nav_tr[[lang()]]
  ...
  # Inicialización del Server de Tamaño Muestral pasándole el parámetro lang
